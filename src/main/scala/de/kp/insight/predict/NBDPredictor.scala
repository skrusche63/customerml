...
class NBDPredictor(ctx:RequestContext,params:Map[String,String]) extends BasePredictor(ctx,params) {
  ...
  override def predict(params:Map[String,String]) {
     ...
     * but in days
     */
    val timestamps = orders.map(_.timestamp).collect().sorted
    
    val begin = timestamps.head
    ...
    val dataset = orders.groupBy(x => (x.site,x.user)).map(p => {
      ...
      val recency = ((latest - bbegin.value) / DAY).toDouble
      val frequency = p._2.size
          
      (site,user,recency,frequency)
    ...
    /*
     * Determine Pareto/NDB factors alpha,beta,r,s
     */
