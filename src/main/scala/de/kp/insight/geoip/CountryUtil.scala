...
object CountryUtil extends Serializable {

  private val countryCodes = Array( "--", "AP", "EU", "AD", "AE",
  ...
  private val countryNames = Array("N/A", "Asia/Pacific Region",
  ...
  private val countries = countryNames.zip(countryCodes).zipWithIndex.map{case((name,code),ix) => ((name,code),ix+1)}
  
  /*
   * Build lookup data structures
   */
  private val country2index = countries.toMap
  private val index2country = countries.map{case((code,name),ix) => (ix,(code,name))}
  
  /**
   * Get index for (code,name) of country
   */
  def getIndex = country2index
  /**
   * Get (code,name) of country from index
   */
  def getCountry = index2country
  /**
   * Get size of all countries
   */
  def getSize = countries.size
