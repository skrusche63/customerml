...
class CARLearner(ctx:RequestContext,params:Map[String,String]) extends BaseActor(ctx) {
  ...
  override def receive = {
    ...
    case message:StartLearn => {
      ...
      val data = new CARHandler(ctx).train(req_params)
       ...
       * factorization machine training request and informs this actor (as parent)
      ...
      /*
       * We evaluate the response message from the remote Context-Aware Analysis 
