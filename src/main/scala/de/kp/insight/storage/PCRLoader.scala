...
/**
 * PCRLoader class directly loads the results of the PCREnricher
 * into the product/recommendations index.
 */
class PCRLoader(ctx:RequestContext,params:Map[String,String]) extends BaseLoader(ctx,params) {
  ...
  override def load(params:Map[String,String]) {
    ...
    if (ctx.putSources("products","recommendations",sources) == false)
