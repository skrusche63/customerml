...
/**
 * ParquetDPT is a data structure that specifies a Parquet
 * file thta describes a feature vector with associated label;
 * it is exclusively used for CAR specific data and does not
 * describe any additional user or item specific fields
 */
case class ParquetDPT(row:Long,col:Long,cat:String,value:Double)
/**
 * ParquetIPoint is a data structure that specifies a Parquet
 * file that describes a feature vector with associated label;
 * it is used to specify the customer's affinity to 
 * 
 * - product
 * - day of the week, 
 * - hour of the day 
 * - timespan between two subsequent transactions
 * 
 * This data structure is shared with Predictiveworks' vector 
 * analysis engines, such as Outlier or Similarity analysis.
 * 
 */
case class ParquetIPT(
  site:String,
  user:String,
